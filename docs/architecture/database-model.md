...
| Table | Key columns | Constraints / indexes |
|---|---|---|
| `customers` | id, agency_id→, kind (INDIVIDUAL/COMPANY), first/last/company name, phone (E.164), whatsapp_same bool, email, segment (DOMESTIC/MRE/TOURIST/BUSINESS), deleted_at | unique(agency_id, phone) where not deleted; idx(agency_id, last_name) |
| `consent_records` | id, agency_id→, customer_id→, purpose (GPS_TRACKING/MARKETING/DATA_PROCESSING), granted bool, language, evidence_object_key, captured_by→, captured_at | idx(customer_id, purpose); CNDP evidence trail (register #3) |
...
| `vehicles` | id, agency_id→, category_id→, model_id→, plate, vin, current_branch_id→, operational_status, fleet_status, current_mileage_km, fuel_level_pct, first_registration_date (age-cap compliance input), acquired_at, deleted_at | unique(agency_id, plate); check (mileage >= 0); idx(operational_status) partial where fleet_status='IN_FLEET' |
| `vehicle_state_transitions` | id, agency_id→, vehicle_id→, from_status, to_status, actor_user_id, reason, source_type, source_id, interrupted_status, created_at | **append-only trigger**; idx(vehicle_id, created_at) |
| `vehicle_signals` | id, agency_id→, vehicle_id→, kind (GHOST_STATE/PHANTOM_BOOKING/UNAUTHORIZED_USE/MAINTENANCE_CONFLICT/PREAUTH_EXPIRING/…), evidence jsonb, first_seen_at, last_evaluated_at, status (OPEN/CLEARED) | idx(agency_id, kind, status); **read model — signals never mutate vehicle status** (ADR-0010) |
| `compliance_rule_sets` | id, agency_id→, min_fleet_size null, age_cap_ice/hybrid/ev null, enabled bool, source_label | values nullable = monitor off; UI labels rules "verify with your accountant" (register #15) |
...
| Table | Key columns | Constraints / indexes |
|---|---|---|
| `payments` | id, agency_id→, direction (IN/OUT), method (CASH/CARD/TRANSFER/ONLINE/DEPOSIT_CASH/REFUND/FATOURATI_LINK later), amount bigint, currency (default MAD; EUR/USD for cash), fx_rate numeric + mad_equivalent bigint (both required when currency ≠ MAD; rate human-confirmed at entry), contract_id→/reservation_id→/invoice_id→/fine_id→ null, reverses_payment_id→, provider_ref, received_by→, received_at, cash_session_id→, meta jsonb | **append-only trigger**; check (amount > 0); check (currency = 'MAD' OR (fx_rate IS NOT NULL AND mad_equivalent IS NOT NULL)); idx(agency_id, received_at); idx(cash_session_id); refunds must reference original (FK reverses_payment_id) |
| `deposits` | id, agency_id→, contract_id→, amount, method (CASH_HELD/CARD_PREAUTH/BANK), provider (e.g. CMI_PLBS) + provider_ref, preauth_expires_at (signal input — research #36), status (PLANNED/HELD/PRE_AUTHORIZED/RELEASED/PARTIALLY_CHARGED/SETTLED), held_by→, released_by→, released_at, release_reason | status transitions via service + audit; idx(contract_id) |
...
| `fines` | id, agency_id→, vehicle_id→, source_document_object_key (NARSA/ANSR PDF), ocr_payload jsonb null (assist only), source_ref, occurred_at, amount, status (RECEIVED/MATCHED/ATTRIBUTED/INVOICED/PAID/DISPUTED), matched_contract_id→ null, attributed_customer_id→, attribution_evidence jsonb, invoice_id→ | idx(vehicle_id, occurred_at); attribution human-confirmed (§14) |
| `cash_sessions` | id, agency_id→, branch_id→, opened_by→, closed_by→, opened_at, closed_at, expected_amount (derived snapshot), counted jsonb (denominations **per currency** — MAD/EUR/USD), counted_total + counted_mad_equivalent, variance (MAD), variance_explanation | one open session per (branch, drawer); idx(branch_id, opened_at desc) |
...
| `maintenance_records` | id, agency_id→, vehicle_id→, task_id→ null, performed_at, garage (INTERNAL/EXTERNAL), garage_name, cost_amount, cost_currency, mileage_km, downtime_hours, documents jsonb (object keys), duration_hours (SLA stat vs task kind — slow-vendor flagging) | idx(vehicle_id, performed_at) |

## 10. Telematics (schema lands with V2; contract fixed now)
...
| `telematics_events` | id, agency_id→, device_id→, event_type (incl. EV: SOC_CHANGED, CHARGING_STATE, CHARGING_FAULT), payload jsonb, occurred_at, provider_message_id | **append-only trigger**; unique(provider, provider_message_id) — idempotent ingestion |
...
| `alerts` | id, agency_id→, rule_id→ (or signal kind), source_kind (RULE/SIGNAL/JOB), dedup_key, severity, evidence jsonb, status (OPEN/ACKNOWLEDGED/RESOLVED/SUPPRESSED), assignee→, resolved_by→, resolution_note, created_at | unique(agency_id, dedup_key) within active window (partial idx); idx(agency_id, status, severity) |
...
- Signal isolation (ADR-0010): injecting a GHOST_STATE-producing telemetry stream must raise
  an alert and **must not** change `vehicles.operational_status` without a human approval.
- Multi-currency: non-MAD payment without `fx_rate`/`mad_equivalent` must be rejected;
  session counted-currency breakdown must reconcile per currency.
