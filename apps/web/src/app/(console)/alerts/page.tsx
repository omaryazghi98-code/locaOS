...
interface Alert { id: string; ruleKey: string; severity: string; status: string; title: string; message: string; createdAt: string; sourceKind: string }
      ...
      <table className="tbl">
        ...
        <tbody>{rules.map((r) => (
          <tr key={r.key}><td>{r.name}<div className="sub">{r.description}</div></td>
            ...
            <td><span className={`pill ${r.severity === 'CRITICAL' ? 'danger' : 'warn'}`}>{r.severity}</span></td>
