...
export default function Fleet() {
  ...
  useEffect(() => {
    ...
    const handleDensity = (event: Event) => setDense((event as CustomEvent<Density>).detail);
    setLang(readLocale());
    setDense(readDensity());
  ...
  const counts = vehicles.reduce<Record<string, number>>((acc, vehicle) => { acc[vehicle.operationalStatus] = (acc[vehicle.operationalStatus] ?? 0) + 1; return acc; }, {});
  ...
  return (
    <>
      <PageHeader title={strings.title} subtitle={loading ? strings.loading : `${vehicles.length} ${strings.vehicles}${vehicles.length > 0 ? ` — ${Object.entries(counts).map(([status, count]) => `${count} ${status}`).join(' · ')}` : ''}`} />
      <Section>
