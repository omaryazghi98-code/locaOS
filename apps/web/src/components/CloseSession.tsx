...
export default function CloseSession({ sessionId, expected }: { sessionId: string; expected: string }) {
  ...
  const close = async () => {
    ...
    if (!res.ok) { const out = await res.json().catch(() => null); setErr(out?.error?.message ?? 'Erreur'); setBusy(false); return; }
    setBusy(false); router.refresh();
  };

  return (
    <div className="card" style={{ maxWidth: 640, marginBottom: 10 }}>
      <h2 style={{ marginTop: 0 }}>Clôture du tiroir-caisse</h2>
      <div className="sub">Attendu : <b>{mad(Number(expected))}</b> — comptez le tiroir par devise. L'écart est calculé et journalisé, jamais écrasé.</div>
      <div className="row" style={{ gap: 16, marginTop: 10 }}>
        <div style={{ flex: 1 }}>
          <label>MAD — pièces/billets</label>
          {DENOMS_MAD.map((d) => (
        ...
        <div style={{ flex: 1 }}>
          <label>EUR (MRE/touristes) — taux du jour (confirmé humain)</label>
      ...
      <div className="grid cards" style={{ marginTop: 10 }}>
        <div className="card"><div className="k">Compté MAD</div><div className="v">{mad(totalMAD)}</div></div>
        <div className="card"><div className="k">EUR → MAD</div><div className="v">{mad(eurInMad)}</div></div>
        <div className="card"><div className="k">Écart</div>
          <div className={`v ${variance === 0 ? 'ok' : 'crit'}`}>{mad(variance)}</div></div>
      </div>
      {variance !== 0 && (<><label>Explication de l'écart (audité)</label><input value={explanation} onChange={(e) => setExplanation(e.target.value)} placeholder="ex: rendu monnaie mal compté" /></>)}
      {err && <div className="error-msg">{err}</div>}
      <div className="btnrow"><button className="primary" onClick={close} disabled={busy}>{busy ? 'Clôture…' : 'Clôturer la session'}</button></div>
    </div>
  );
}
