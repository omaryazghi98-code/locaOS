...
export class ContractsController {
  ...
  async list(@Req() req: AuthedRequest) {
    return withTenant(req.ctx!.agencyId, async (tx) => {
      const rows = await tx.select({ c: contracts, customerName: sql<string>`${customers.firstName} || ' ' || ${customers.lastName}`, plate: vehicles.plate })
        .from(contracts).innerJoin(customers, eq(customers.id, contracts.customerId)).leftJoin(vehicles, eq(vehicles.id, contracts.vehicleId))
        .where(eq(contracts.agencyId, req.ctx!.agencyId)).orderBy(desc(contracts.createdAt)).limit(200);
  ...
  async issueBlank(@Body(new ZodValidationPipe(z.object({ language: z.enum(['fr', 'ar', 'en']) }))) body: { language: 'fr' | 'ar' | 'en' }, @Req() req: AuthedRequest) {
    const result = await withTenant(req.ctx!.agencyId, async (tx) => {
      ...
      const inserted = await tx.insert(contracts).values({ agencyId: req.ctx!.agencyId, number, language: body.language, status: 'BLANK_ISSUED', blankIssuedAt: new Date() }).returning();
      const versionId = await newVersion(tx, req.ctx!.agencyId, inserted[0]!.id, content, req.ctx!.userId);
      await audit(tx, { agencyId: req.ctx!.agencyId, actor: { id: req.ctx!.userId, name: req.ctx!.fullName }, entityType: 'contract', entityId: inserted[0]!.id, action: 'BLANK_CONTRACT_ISSUED', after: { number, language: body.language } });
  ...
  @RequirePermission('contracts:blank')
  async reconcileBlank(@Param('id', ParseUUIDPipe) id: string, @Body(new ZodValidationPipe(z.object({ reservationId: z.string().uuid(), scannedObjectKey: z.string().optional() }))) body: { reservationId: string; scannedObjectKey?: string }, @Req() req: AuthedRequest) {
    const result = await withTenant(req.ctx!.agencyId, async (tx) => {
      ...
      const updated = await tx.update(contracts).set({ status: 'DRAFT', reservationId: body.reservationId, customerId: data.reservation!.customerId, vehicleId: data.reservation!.vehicleId, branchId: data.reservation!.branchOutId, periodStart: data.reservation!.pickupAt, periodEnd: data.reservation!.returnAt, reconciledAt: new Date(), scannedObjectKey: body.scannedObjectKey ?? contract.scannedObjectKey, updatedAt: new Date() }).where(eq(contracts.id, id)).returning();
      await audit(tx, { agencyId: req.ctx!.agencyId, actor: { id: req.ctx!.userId, name: req.ctx!.fullName }, entityType: 'contract', entityId: id, action: 'BLANK_CONTRACT_RECONCILED', before: { status: 'BLANK_ISSUED' }, after: { status: 'DRAFT', reservationId: body.reservationId } });
  ...
  async void(@Param('id', ParseUUIDPipe) id: string, @Body(new ZodValidationPipe(z.object({ reason: z.string().min(3).max(300) }))) body: { reason: string }, @Req() req: AuthedRequest) {
    const result = await withTenant(req.ctx!.agencyId, async (tx) => {
      ...
      const updated = await tx.update(contracts).set({ status: 'VOIDED', voidedReason: body.reason, updatedAt: new Date() }).where(eq(contracts.id, id)).returning();
      await audit(tx, { agencyId: req.ctx!.agencyId, actor: { id: req.ctx!.userId, name: req.ctx!.fullName }, entityType: 'contract', entityId: id, action: 'CONTRACT_VOIDED', before: { status: contract.status }, after: { status: 'VOIDED' }, reason: body.reason });
  ...
  async fromReservation(@Body(new ZodValidationPipe(z.object({ reservationId: z.string().uuid(), language: z.enum(['fr', 'ar', 'en']).default('fr') }))) body: { reservationId: string; language: 'fr' | 'ar' | 'en' }, @Req() req: AuthedRequest) {
    const result = await withTenant(req.ctx!.agencyId, async (tx) => {
      const r = await tx.select().from(reservations).where(and(eq(reservations.id, body.reservationId), eq(reservations.agencyId, req.ctx!.agencyId))).limit(1);
      if (!r[0]) throw new NotFoundException('Réservation introuvable');
      const existing = await tx.select().from(contracts).where(and(eq(contracts.reservationId, body.reservationId), inArray(contracts.status, ['DRAFT', 'SIGNED', 'ACTIVE', 'AMENDED']))).limit(1);
      if (existing[0]) return { contract: existing[0], existing: true };
      if (!r[0].vehicleId) throw new ConflictException({ error: { code: 'VEHICLE_REQUIRED_FOR_CONTRACT', message: 'Un véhicule doit être affecté avant de préparer le contrat' } });
      ...
      const inserted = await tx.insert(contracts).values({ agencyId: req.ctx!.agencyId, number, reservationId: body.reservationId, customerId: r[0].customerId, vehicleId: r[0].vehicleId, branchId: r[0].branchOutId, language: body.language, status: 'DRAFT', periodStart: r[0].pickupAt, periodEnd: r[0].returnAt }).returning();
      await newVersion(tx, req.ctx!.agencyId, inserted[0]!.id, content, req.ctx!.userId);
      await audit(tx, { agencyId: req.ctx!.agencyId, actor: { id: req.ctx!.userId, name: req.ctx!.fullName }, entityType: 'contract', entityId: inserted[0]!.id, action: 'CONTRACT_GENERATED', after: { number, reservationId: body.reservationId, language: body.language } });
  ...
  async sign(@Param('id', ParseUUIDPipe) id: string, @Body(new ZodValidationPipe(z.object({ customerName: z.string().min(2).max(120), gpsConsent: z.boolean().default(false) }))) body: { customerName: string; gpsConsent: boolean }, @Req() req: AuthedRequest) {
    const result = await withTenant(req.ctx!.agencyId, async (tx) => {
      ...
      content.signatures = { customer: { present: true, name: body.customerName, at: new Date().toISOString() }, agent: { present: true, name: req.ctx!.fullName, at: new Date().toISOString() } };
      ...
      await audit(tx, { agencyId: req.ctx!.agencyId, actor: { id: req.ctx!.userId, name: req.ctx!.fullName }, entityType: 'contract', entityId: id, action: 'CONTRACT_SIGNED', before: { status: contract.status }, after: { status: 'SIGNED' } });
  ...
  async activate(@Param('id', ParseUUIDPipe) id: string, @Req() req: AuthedRequest) {
    return withTenant(req.ctx!.agencyId, async (tx) => {
      ...
      const conflictingReservation = await tx.select({ reference: reservations.reference }).from(reservations).where(and(eq(reservations.agencyId, req.ctx!.agencyId), eq(reservations.vehicleId, vehicle.id), sql`${reservations.id} <> ${reservation.id}`, inArray(reservations.status, ['CONFIRMED', 'VEHICLE_ASSIGNED', 'READY', 'IN_PROGRESS'] as any), sql`${reservations.pickupAt} < ${reservation.returnAt}`, sql`${reservations.returnAt} > ${reservation.pickupAt}`)).limit(1);
      if (conflictingReservation[0]) throw new ForbiddenException(`Véhicule déjà engagé sur ${conflictingReservation[0].reference}`);
      const departure = await tx.select().from(inspections).where(and(eq(inspections.reservationId, reservation.id), eq(inspections.kind, 'DEPARTURE'))).limit(1);
      if (!departure[0]) throw new ForbiddenException('Inspection départ obligatoire avant remise');
      const quote = reservation.quoteId ? (await tx.select().from(quotes).where(eq(quotes.id, reservation.quoteId)).limit(1))[0] : null;
      const requiredDeposit = quote?.depositRequired ?? 0n;
      if (requiredDeposit > 0n) {
        const secured = await tx.select().from(deposits).where(and(eq(deposits.agencyId, req.ctx!.agencyId), eq(deposits.contractId, contract.id), inArray(deposits.status, ['HELD', 'PRE_AUTHORIZED', 'PARTIALLY_CHARGED']))).limit(1);
        const securedAmount = secured[0]?.amount ?? 0n;
        assertDepositCoversRequiredAmount(securedAmount, requiredDeposit);
      }
      const updated = await tx.update(contracts).set({ status: 'ACTIVE', updatedAt: new Date() }).where(and(eq(contracts.id, id), eq(contracts.status, 'SIGNED'))).returning();
      ...
      await transitionVehicle(tx, req.ctx!.agencyId, { vehicleId: contract.vehicleId, to: 'RENTED', actorId: req.ctx!.userId, actorName: req.ctx!.fullName, actorKind: 'CONTRACT_SERVICE', reason: `Remise du véhicule — contrat ${contract.number}`, sourceType: 'contract', sourceId: id });
      ...
      await audit(tx, { agencyId: req.ctx!.agencyId, actor: { id: req.ctx!.userId, name: req.ctx!.fullName }, entityType: 'contract', entityId: id, action: 'CONTRACT_ACTIVATED', before: { status: 'SIGNED' }, after: { status: 'ACTIVE' } });
      return updated[0];
    });
  }
  ...
  async close(@Param('id', ParseUUIDPipe) id: string, @Req() req: AuthedRequest) {
    const contractRows = await withTenant(req.ctx!.agencyId, async (tx) => {
      ...
      if (!contract.reservationId || !contract.vehicleId) throw new ConflictException({ error: { code: 'RETURN_FLOW_INCOMPLETE', message: 'Réservation et véhicule requis pour clôturer le contrat' } });
      const reservation = (await tx.select().from(reservations).where(and(eq(reservations.id, contract.reservationId), eq(reservations.agencyId, req.ctx!.agencyId))).limit(1))[0];
      if (!reservation) throw new NotFoundException('Réservation introuvable');
  ...
  async amend(@Param('id', ParseUUIDPipe) id: string, @Body(new ZodValidationPipe(z.object({ kind: z.enum(['VEHICLE_REPLACEMENT', 'PERIOD', 'DRIVER', 'PRICE', 'OTHER']), reason: z.string().min(3).max(300), newVehicleId: z.string().uuid().optional(), newReturnAt: z.string().datetime().optional(), newDriver: z.object({ name: z.string().max(120), licenseNumber: z.string().max(40), birthDate: z.string().date().optional() }).optional(), newDailyRate: z.number().optional() }))) body: { kind: 'VEHICLE_REPLACEMENT' | 'PERIOD' | 'DRIVER' | 'PRICE' | 'OTHER'; reason: string; newVehicleId?: string; newReturnAt?: string; newDriver?: { name: string; licenseNumber: string; birthDate?: string }; newDailyRate?: number }, @Req() req: AuthedRequest) {
    if (body.kind === 'PRICE' && !req.ctx!.permissions.has('contract:price:override')) throw new ForbiddenException('Permission contract:price:override requise');
    const result = await withTenant(req.ctx!.agencyId, async (tx) => {
      const contract = await loadContract(tx, req.ctx!.agencyId, id);
      ...
      if (body.kind === 'VEHICLE_REPLACEMENT') {
        ...
        const v = await tx.select().from(vehicles).where(and(eq(vehicles.id, body.newVehicleId), eq(vehicles.agencyId, req.ctx!.agencyId))).limit(1);
        ...
        content.vehicle = { plate: v[0].plate, makeModel: m[0] ? `${m[0].make} ${m[0].model} (${m[0].year})` : null, category: content.vehicle.category, mileageOut: String(v[0].currentMileageKm), fuelOut: `${v[0].fuelLevelPct}%`, vin: v[0].vin };
        ...
        await tx.update(contracts).set({ status: 'AMENDED', updatedAt: new Date() }).where(eq(contracts.id, id));
        await appendEvent(tx, req.ctx!.agencyId, 'PriceOverridden', { contractId: id, reservationId: contract.reservationId, from: oldRate ?? '0', to: String(body.newDailyRate), reason: body.reason });
      } else {
        await tx.update(contracts).set({ status: 'AMENDED', updatedAt: new Date() }).where(eq(contracts.id, id));
      }
      const versionId = await newVersion(tx, req.ctx!.agencyId, id, content, req.ctx!.userId);
      const amendment = await tx.insert(contractAmendments).values({ agencyId: req.ctx!.agencyId, contractId: id, kind: body.kind, payload: body as never, reason: body.reason, createdBy: req.ctx!.userId, resultingVersionId: versionId }).returning();
      await audit(tx, { agencyId: req.ctx!.agencyId, actor: { id: req.ctx!.userId, name: req.ctx!.fullName }, entityType: 'contract', entityId: id, action: 'CONTRACT_AMENDED', after: { kind: body.kind }, reason: body.reason });
  ...
  async uploadScan(@Param('id', ParseUUIDPipe) id: string, @UploadedFile() file: Express.Multer.File, @Req() req: AuthedRequest) {
    if (!file) throw new ForbiddenException('Fichier requis');
    ...
    return withTenant(req.ctx!.agencyId, async (tx) => {
      const updated = await tx.update(contracts).set({ scannedObjectKey: key, updatedAt: new Date() }).where(and(eq(contracts.id, id), eq(contracts.agencyId, req.ctx!.agencyId))).returning();
      if (!updated[0]) throw new NotFoundException('Contrat introuvable');
      await audit(tx, { agencyId: req.ctx!.agencyId, actor: { id: req.ctx!.userId, name: req.ctx!.fullName }, entityType: 'contract', entityId: id, action: 'PAPER_SCAN_ATTACHED', after: { key } });
...
async function loadAssemblyData(tx: Tx, agencyId: string, reservationId: string, vehicleId: string | null) {
  const r = await tx.select().from(reservations).where(and(eq(reservations.id, reservationId), eq(reservations.agencyId, agencyId))).limit(1);
  if (!r[0]) throw new NotFoundException('Réservation introuvable');
