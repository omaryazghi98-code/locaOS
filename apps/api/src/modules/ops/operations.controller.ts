...
const CreateSchema = z.object({
  vehicleId: z.string().uuid(), reservationId: z.string().uuid().optional(), contractId: z.string().uuid().optional(),
  taskKind: TaskKind, title: z.string().min(2).max(160), description: z.string().max(1000).optional(),
  priority: z.enum(['LOW', 'NORMAL', 'HIGH', 'URGENT']).default('NORMAL'), assignedTo: z.string().uuid().optional(),
  vendorId: z.string().uuid().optional(), scheduledStart: z.string().datetime().optional(), scheduledEnd: z.string().datetime().optional(),
  estimatedCost: z.string().optional(),
});

const TriageSchema = z.object({
  cleaningNeeded: z.boolean().default(false), maintenanceNeeded: z.boolean().default(false),
  cleaningNote: z.string().max(500).optional(), maintenanceNote: z.string().max(500).optional(),
});

const UpdateSchema = z.object({
  status: TaskStatus.optional(), assignedTo: z.string().uuid().nullable().optional(), vendorId: z.string().uuid().nullable().optional(),
  completionNote: z.string().max(1000).optional(), actualCost: z.string().optional(), evidence: z.record(z.string(), z.unknown()).optional(),
...
export class OperationsController {
  ...
  async list(@Req() req: AuthedRequest) {
    return withTenant(req.ctx!.agencyId, async (tx) => {
      const result = await tx.execute(sql`
        select t.*, v.plate, coalesce(u.full_name, '') as assignee_name
          from operations_tasks t join vehicles v on v.id = t.vehicle_id
          left join users u on u.id = t.assigned_to
         where t.agency_id = ${req.ctx!.agencyId}
         order by case t.priority when 'URGENT' then 0 when 'HIGH' then 1 when 'NORMAL' then 2 else 3 end, t.created_at desc limit 300`);
  ...
  async detail(@Param('id', ParseUUIDPipe) id: string, @Req() req: AuthedRequest) {
    return withTenant(req.ctx!.agencyId, async (tx) => {
      const result = await tx.execute(sql`select t.*, v.plate from operations_tasks t join vehicles v on v.id=t.vehicle_id where t.agency_id=${req.ctx!.agencyId} and t.id=${id} limit 1`);
  ...
  async create(@Body(new ZodValidationPipe(CreateSchema)) body: z.infer<typeof CreateSchema>, @Req() req: AuthedRequest) {
    return withTenant(req.ctx!.agencyId, async (tx) => {
      const result = await tx.execute(sql`
        insert into operations_tasks (agency_id,vehicle_id,reservation_id,contract_id,task_kind,title,description,priority,status,assigned_to,vendor_id,scheduled_start,scheduled_end,estimated_cost,created_by)
        values (${req.ctx!.agencyId},${body.vehicleId},${body.reservationId??null},${body.contractId??null},${body.taskKind},${body.title},${body.description??null},${body.priority},'OPEN',${body.assignedTo??null},${body.vendorId??null},${body.scheduledStart?new Date(body.scheduledStart):null},${body.scheduledEnd?new Date(body.scheduledEnd):null},${body.estimatedCost!=null?BigInt(Math.round(Number(body.estimatedCost)*100)):null},${req.ctx!.userId}) returning *`);
  ...
  async triageReturn(@Param('id',ParseUUIDPipe) id:string,@Body(new ZodValidationPipe(TriageSchema)) body:z.infer<typeof TriageSchema>,@Req() req:AuthedRequest){
    const result=await withTenant(req.ctx!.agencyId,async(tx)=>{
      const found=await tx.execute(sql`select * from operations_tasks where agency_id=${req.ctx!.agencyId} and id=${id} for update`);
      ...
      if(!['OPEN','ASSIGNED','IN_PROGRESS'].includes(review.status)) throw new ConflictException('La revue de préparation est déjà traitée');
      const children:any[]=[];
      if(body.cleaningNeeded){const r=await tx.execute(sql`insert into operations_tasks(agency_id,vehicle_id,reservation_id,contract_id,source_inspection_id,task_kind,title,description,priority,status,created_by) values(${review.agency_id},${review.vehicle_id},${review.reservation_id},${review.contract_id},${review.source_inspection_id},'CLEANING','Nettoyage / préparation esthétique',${body.cleaningNote??null},'NORMAL','OPEN',${req.ctx!.userId}) returning *`);children.push((r as unknown as {rows:any[]}).rows[0]);}
      if(body.maintenanceNeeded){const r=await tx.execute(sql`insert into operations_tasks(agency_id,vehicle_id,reservation_id,contract_id,source_inspection_id,task_kind,title,description,priority,status,created_by) values(${review.agency_id},${review.vehicle_id},${review.reservation_id},${review.contract_id},${review.source_inspection_id},'MAINTENANCE','Contrôle / réparation mécanique',${body.maintenanceNote??null},'HIGH','OPEN',${req.ctx!.userId}) returning *`);children.push((r as unknown as {rows:any[]}).rows[0]);}
      await tx.execute(sql`update operations_tasks set status='COMPLETED',completion_note=${children.length?'Triage terminé — travaux créés.':'Aucun travail supplémentaire requis.'},completed_at=now(),completed_by=${req.ctx!.userId},updated_at=now() where id=${id} and agency_id=${req.ctx!.agencyId}`);
      if(children.some(t=>t.task_kind==='CLEANING')) await transitionVehicle(tx,req.ctx!.agencyId,{vehicleId:review.vehicle_id,to:'CLEANING',actorId:req.ctx!.userId,actorName:req.ctx!.fullName,actorKind:'OPS_SERVICE',reason:'Préparation post-retour — nettoyage requis',sourceType:'operations_task',sourceId:id});
      else if(children.some(t=>t.task_kind==='MAINTENANCE')) await transitionVehicle(tx,req.ctx!.agencyId,{vehicleId:review.vehicle_id,to:'MAINTENANCE',actorId:req.ctx!.userId,actorName:req.ctx!.fullName,actorKind:'OPS_SERVICE',reason:'Préparation post-retour — maintenance requise',sourceType:'operations_task',sourceId:id});
      else await transitionVehicle(tx,req.ctx!.agencyId,{vehicleId:review.vehicle_id,to:'AVAILABLE',actorId:req.ctx!.userId,actorName:req.ctx!.fullName,actorKind:'OPS_SERVICE',reason:'Retour inspecté — aucune préparation supplémentaire requise',sourceType:'operations_task',sourceId:id});
    ...
    dispatchPendingSafe(); return result;
  }
  ...
  async update(@Param('id',ParseUUIDPipe) id:string,@Body(new ZodValidationPipe(UpdateSchema)) body:z.infer<typeof UpdateSchema>,@Req() req:AuthedRequest){
    const result=await withTenant(req.ctx!.agencyId,async(tx)=>{
      ...
      if(nextStatus==='COMPLETED'&&current.task_kind==='QA'){
        const open=await tx.execute(sql`select count(*)::int as n from operations_tasks where agency_id=${req.ctx!.agencyId} and vehicle_id=${current.vehicle_id} and status in ('OPEN','ASSIGNED','IN_PROGRESS','BLOCKED')`);
        ...
        if(n===0){
          ...
          if(status==='CLEANING'||status==='MAINTENANCE') await transitionVehicle(tx,req.ctx!.agencyId,{vehicleId:current.vehicle_id,to:'AVAILABLE',actorId:req.ctx!.userId,actorName:req.ctx!.fullName,actorKind:'OPS_SERVICE',reason:'Contrôle qualité réussi — aucune tâche post-retour restante',sourceType:'operations_task',sourceId:id});
        }
      }
