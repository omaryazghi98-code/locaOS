...
export interface TransitionRule { from: VehicleStatus | 'ANY'; to: VehicleStatus; actors: TransitionActor[] | 'ANY'; reasonRequired?: boolean; note?: string; }
export const TRANSITIONS: readonly TransitionRule[] = [
  ...
  // Availability after return is an operations outcome, never an agent action.
  // OPS_SERVICE is only allowed to make the transition after the task layer proves the gate.
...
export type TransitionErrorCode = 'SAME_STATE' | 'UNKNOWN_TRANSITION' | 'ACTOR_NOT_ALLOWED' | 'REASON_REQUIRED' | 'SYSTEM_ONLY';
export interface TransitionCheck { ok: boolean; code?: TransitionErrorCode; message?: string; rule?: TransitionRule; }
export function checkTransition(from: VehicleStatus, to: VehicleStatus, actor: TransitionActor, opts: { reason?: string } = {}): TransitionCheck {
  if (from === to) return { ok: false, code: 'SAME_STATE', message: `Vehicle is already ${from}` };
  const rule = TRANSITIONS.find((r) => r.to === to && (r.from === from || r.from === 'ANY'));
  if (!rule) return { ok: false, code: 'UNKNOWN_TRANSITION', message: `Illegal transition ${from} → ${to}` };
  if (rule.actors !== 'ANY' && !rule.actors.includes(actor)) return { ok: false, code: 'ACTOR_NOT_ALLOWED', message: `Actor ${actor} may not perform ${from} → ${to}` };
  if (rule.reasonRequired && !opts.reason?.trim()) return { ok: false, code: 'REASON_REQUIRED', message: `A reason is required for ${from} → ${to}` };
  if (to === 'OVERDUE' && actor !== 'SCHEDULER') return { ok: false, code: 'SYSTEM_ONLY', message: 'OVERDUE is entered only by the scheduled evaluator' };
  return { ok: true, rule };
}
export const PIPELINE_STATES: readonly VehicleStatus[] = ['RESERVED', 'PREPARING', 'CONTRACT_READY', 'IN_TRANSIT', 'RENTED', 'OVERDUE', 'AWAITING_INSPECTION', 'INSPECTED', 'CLEANING'];
